...
| All-to-all without ordering | Porting DeepEP's MoE dispatch/combine to a fabric that does not preserve order (AWS EFA / SRD) — who writes the WQE across ibverbs, libfabric, NVSHMEM, NCCL GIN and EFA-GDA; where the upstream tail-pointer scheme breaks; the in-band header plus counting-signal protocol and the three invariants its correctness rests on; how a 512-signal budget produces a 176-rank ceiling; and whether the whole thing can be verified without an EFA instance | [deepep-efa-index.html](./deepep-efa-index.html) |
